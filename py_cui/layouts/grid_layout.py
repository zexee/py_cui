...
class GridLayout(Layout):
  ...
  def _refresh_height_width(self, height, width):
    ...
    for _, ww in self._widgets.items():
      w, row, col, row_span, col_span = ww
  ...
  def _is_row_col_inside(self, id, row, col):
      w, w_row, w_col, w_row_span, w_col_span = self._widgets[id]
      return w_row <= row and row < w_row + w_row_span and w_col <= col and col < w_col + w_col_span


  def _get_vertical_neighbor(self, row, col, row_span, col_span, direction):
    start, end, step = (row - 1, 0, -1) if direction == py_cui.keys.KEY_UP_ARROW else (row + row_span, self._num_rows, 1)
  ...
  def _get_horizontal_neighbor(self, row, col, row_span, col_span, direction):
    start, end, step = (col - 1, 0, -1) if direction == py_cui.keys.KEY_LEFT_ARROW else (col + col_span, self._num_cols, 1)
  ...
  def add_widget(self, widget, row, col, row_span=1, col_span=1):
    ...
    if self._selected_widget is None and widget._style['selectable']:
      self._selected_widget = widget._id
    return widget
